...
- The viewers now wrap long lines by default, so a JSON body on a single line is readable without altering the text: what a selection copies out of the viewer is exactly what came over the wire. Turning "Wrap lines" off falls back to breaking lines past 4000 characters for display only — with the wrap off Swing lays a line out as one run and past ~32767 pixels the glyphs pile up into a smear — and never in what is copied or saved
