...
- "Start on Boot" now actually starts the tunnel when the IDE opens the project. It used to wait for the tool window to be opened by hand, because the platform builds a tool window's content only on first use and that is where the auto-start lives; the content is now built as the project opens when the flag is set, without the panel popping open
